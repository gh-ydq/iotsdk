...
public class LightDeviceCtl {
	private Logger logger = LoggerFactory.getLogger(LightDeviceCtl.class);
	static int i_seq = 0;

	public boolean lightCmd(long imei, LightCmdEnum lightCmdEnum) {
		logger.info("led 开灯 imei={},lightCmdEnum={}",imei,lightCmdEnum.getCmd());
		CmdBuilder builder = new CmdBuilder.Builder()
				.busiType(lightCmdEnum.getBusiType())
				.imei(imei)
				.seq(CmdSeqUtil.gneratorCmdSeq())
				.cmd(lightCmdEnum.getCmd())
				.param(lightCmdEnum.getParam()).build();
		byte[]  cmd = builder.toCmd();
		return Connection.getInstance().sendCmd(cmd);
	}

	public boolean lightCmd(long imei, LightCmdEnum lightCmdEnum,String ip,int port) {
		logger.info("led 开灯 imei={},lightCmdEnum={}",imei,lightCmdEnum.getCmd());
		CmdBuilder builder = new CmdBuilder.Builder()
				.busiType(lightCmdEnum.getBusiType())
				.imei(imei)
				...
				.param(lightCmdEnum.getParam()).build();
		byte[]  cmd = builder.toCmd();
		return Connection.getInstance(ip,port).sendCmd(cmd);
	}
